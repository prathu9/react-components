...
export const Calendar = (props: CalendarPropsType) => {
  ...
  const [visibleMonthList, setVisibleMonthList] = useState<number[]>([]);
  const listRef: React.LegacyRef<VariableSizeList<any>> | undefined = useRef(null);
  ...
  }, [setScrollArea, scroll]);

  const prevProps = usePrevious(props);

  useEffect(() => {
    // setPreview(props.preview);
    if (scroll.enabled && listRef) {
  ...
  useEffect(() => {
    if(prevProps){
      ...
      if (
        ...
        prevProps[targetProp as keyof typeof prevProps]
      ) {
      ...
      if (
        ...
        prevProps?.weekStartsOn !== props.weekStartsOn
      ) {
        dateOptions = { locale };
      ...
      if (!shallowEqual(prevProps?.scroll, props.scroll)) {
  ...
  const updateShownDate = (props: any) => {
    const newProps = props.scroll?.enabled
      ? {
          ...props,
          months: visibleMonthList.length,
        }
      : props;

    const newFocus = calcFocusDate(focusedDate, newProps);
    focusToDate(newFocus, newProps.scroll);
  };
  ...
  const handleScroll = () => {
    ...
    const visibleMonths: number[] = visibleMonthList ? visibleMonthList:[];
  ...
  const focusToDate = (date: any, scroll?: any, preventUnnecessary = true) => {
    ...
    let targetMonthIndex = dayjs(`${date.getFullYear()}-${date.getMonth()+1}-01`).diff("2022-11-01", "month", true);
    const visibleMonths: number[] = visibleMonthList ? visibleMonthList:[];
    if (preventUnnecessary && visibleMonths?.includes(targetMonthIndex)) return;
    isFirstRender = true;
    listRef?.current?.scrollToItem(targetMonthIndex);
    setFocusedDate(date);
  };
  ...
  const estimateMonthSize = (index: any, cache?: any) => {
    ...
    if (scrollArea) {
        ...
        const isLongMonth = dayjs(end).diff(start, 'days')+ 1 > 7 * 5;
       return isLongMonth
            ? scrollArea?.longMonthHeight! + 20
  ...
  const rowHeights = useMemo(
    () =>
      new Array(
        dayjs(dayjs(maxDate).endOf("month").toDate()).diff(
          ...
          <Box>
            ...
            <Box
              ...
              style={{
                ...
                height:
                  // eslint-disable-next-line no-unsafe-optional-chaining
                  scrollArea?.calendarHeight && scrollArea?.calendarHeight + 50,
              }}
            >
              <List
                height={290}
                itemCount={dayjs(dayjs(maxDate).endOf("month").toDate()).diff(
                  dayjs(minDate).startOf("month").add(-1).toDate(),
                  "month",
                )}
                ref={listRef}
                ...
                onItemsRendered={({
                  ...
                  setVisibleMonthList([visibleStartIndex, visibleStopIndex]);
                }}
              >
                {ItemRenderer}
              </List>
